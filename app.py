...
try:
    ...
    if GOOGLE_API_KEY:
        ...
        print(f"✅ Google AI configured successfully. Key found: {GOOGLE_API_KEY[:5]}...")
    else:
        print("⚠️ GOOGLE_API_KEY not found in .env file.")
except ImportError:
    print("⚠️ google-generativeai library not installed. Run: pip install -r requirements.txt")
...
def process_chat_message(message):
    ...
    if genai is None or not GOOGLE_API_KEY:
        print("Bot Fallback: Gemini not available")
        return {
            "text": "I can still help you search for products — what would you like to find?",
            "action": None
        }

    try:   
        # UPDATED: Using a newer, less busy model
        ...
        if "quota" in error_str or "429" in error_str or "resource exhausted" in error_str:
            return {
                "text": "AI is temporarily unavailable due to high usage. Try searching for a product directly!",
                "action": None
            }

        return {
            "text": "I'm having trouble connecting to AI right now. But I can still help you search for products!",
            "action": None
        }
...
if __name__ == '__main__':
    ...
    print(f"🚀 Starting PriceScraper Pro on port {port} (debug={debug_mode})")
